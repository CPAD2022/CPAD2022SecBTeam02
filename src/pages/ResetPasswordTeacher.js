...
export default class TeacherResetPassword extends Component {
    state={
        email:'',
        oldPassword: '',
        password:'',
    ...
    onBottonPress(){
        ...
        db.collection('teacher').where("email", '==',`${this.state.email}`.toLowerCase()).where("password", '==', this.state.oldPassword).get().then(snapshot =>{
            if(snapshot.docs.length > 0 ){
                const teacher = snapshot.docs[0].id
                db.collection('teacher').doc(teacher).update({
