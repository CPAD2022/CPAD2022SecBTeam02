...
export default class SelectInternalsForMarks extends Component {
  ...
  <View style={styles.maincontainer}>
    ...
    {this.state.isLoading || this.state.selectedBatch == undefined  ? <ActivityIndicator size="large" color="#1273de" /> : 
    ...
    {this.state.isLoading || this.state.selectedSubject == undefined ? <ActivityIndicator size="small" color="#1273de" /> : 
    ...
    onChangeText={(v) => {
      ...
      this.state["selectedInternal"] = "No internals present"
    ...
    useNativeDriver={true}
    value={this.state.selectedInternal}
    onChangeText={(v) => {
      this.setState({
        selectedInternal: v
      })
...
/>
   ...
   <TouchableOpacity  style={styles.button} 
      onPress={() => Actions.getmarks({internal: this.state.selectedInternal})}>
