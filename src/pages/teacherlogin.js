...
export default class Teacherlogin extends Component {
    ...
    onBottonPress(){
        ...
        db.collection('teacher').where("email", '==',this.state.Email).where("password", '==', this.state.password).get().then(snapshot =>{
            if(snapshot.docs.length > 0 ){
                Actions.teacherhomepage()  
                }
            else{
    ...
    render(){
        return (
            ...
            <ImageBackground 
                ...
                <Logo />
                <Text style={styles.logintext}>Teacher, Welcome to login screen  </Text>
